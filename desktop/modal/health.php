...
/*
	DEPLACE DANS 	public static function ScanAmazonAlexa($_logical_id = null, $_exclusion = 0) de core class alexaspotify.class.php
...
foreach($json as $item)
	{
		...
		$alexaspotify = alexaspotify::byLogicalId($serial, 'alexaspotify');
		if (!is_object($alexaspotify)) {
			$alexaspotify = new alexaspotify();
			$alexaspotify->setName($device);
			$alexaspotify->setLogicalId($serial); 
			$alexaspotify->setEqType_name('alexaspotify');
			$alexaspotify->setIsEnable(1);
			$alexaspotify->setIsVisible(1);
		}
		$alexaspotify->setConfiguration('serial',$serial); 
		$alexaspotify->setConfiguration('device',$device);
		$alexaspotify->setConfiguration('type',$type);
		$alexaspotify->setStatus('online',$online);
		$alexaspotify->save();
 }
*/

$eqLogics = alexaspotify::byType('alexaspotify');
?>
