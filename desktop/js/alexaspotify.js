        ...
        buttons: {
            ...
            success: {
                ...
                callback: function () {
                    $.ajax({
                        type: "POST",
                        url: "plugins/alexaspotify/core/ajax/alexaspotify.ajax.php",
                        data: {
...
$('#bt_addEvent').off('click').on('click', function () {
    ...
    $('#md_modal').load('index.php?v=d&plugin=alexaspotify&modal=alarm&eqLogic_id=' + $('.eqLogicAttr[data-l1key=id]').value()).dialog('open');
});

$('#bt_media').off('click').on('click', function () {
    $('#md_modal').dialog({title: "{{Info Media}}"});
    $('#md_modal').load('index.php?v=d&plugin=alexaspotify&modal=media&iddevice=' + $('.eqLogicAttr[data-l1key=logicalId]').value()).dialog('open');
});
$('#bt_req').off('click').on('click', function () {
    $('#md_modal').dialog({title: "{{Requêteur JSON}}"});
    $('#md_modal').load('index.php?v=d&plugin=alexaspotify&modal=req&iddevice=' + $('.eqLogicAttr[data-l1key=logicalId]').value()).dialog('open');
});
$('#bt_req2').off('click').on('click', function () {
    $('#md_modal').dialog({title: "{{Requêteur}}"});
    $('#md_modal').load('index.php?v=d&plugin=alexaspotify&modal=req2&iddevice=' + $('.eqLogicAttr[data-l1key=logicalId]').value()).dialog('open');
});
$('#bt_sante').off('click').on('click', function () {
    $('#md_modal').dialog({title: "{{Liste Players Spotify}}"});
    $('#md_modal').load('index.php?v=d&plugin=alexaspotify&modal=health').dialog('open');
});

$('#bt_reminders').off('click').on('click', function () {
    $('#md_modal').dialog({title: "{{Rappels/Alarmes}}"});
    $('#md_modal').load('index.php?v=d&plugin=alexaspotify&modal=reminders').dialog('open');
});

$('#bt_history').off('click').on('click', function () {
    $('#md_modal').dialog({title: "{{Historique}}"});
    $('#md_modal').load('index.php?v=d&plugin=alexaspotify&modal=history').dialog('open');
});

$('#bt_routines').off('click').on('click', function () {
    $('#md_modal').dialog({title: "{{Routines}}"});
    $('#md_modal').load('index.php?v=d&plugin=alexaspotify&modal=routines').dialog('open');
});
...
$('.eqLogicAttr[data-l1key=configuration][data-l2key=type]').on('change', function () {
    // Ici il faudra faire comme dans desktop/php/alexaspotify.php ligne 268 chercher type et à défaut family
    ...
    if (id) {
        jeedom.eqLogic.byId({
            "id": $('.eqLogicAttr[data-l1key=id]')[0].value, noCache: true, success: function (obj) {
...
jeedom.plugin.getDeamonCookieInfo = function (_params) {
    ...
    paramsAJAX.url = 'plugins/alexaspotify/core/ajax/alexaspotify.ajax.php';
    paramsAJAX.data = {
...
jeedom.plugin.node.action = function (_params) { //Delete reminder
    ...
    var params = $.extend({}, jeedom.private.default_params, paramsSpecifics, _params || {});
    var paramsAJAX = jeedom.private.getParamsAJAX(params);
    paramsAJAX.url = 'plugins/alexaspotify/desktop/php/alexaspotifyProxy.php';
    paramsAJAX.data = {
